...
`endif
   ...
   reg        DE_nop;
   reg [4:0]  DE_rdId;
   (* max_fanout = 32 *) reg [4:0]  DE_rs1Id;
   (* max_fanout = 32 *) reg [4:0]  DE_rs2Id;
   reg [1:0]  DE_csrId;
   reg [2:0]  DE_funct3;
   ...
   reg DE_isALUreg;
   reg DE_isALUimm;
   (* max_fanout = 32 *) reg DE_isBranch;
   (* max_fanout = 32 *) reg DE_isJALR;
   reg DE_isJAL;
   reg DE_isAUIPC;
   reg DE_isLUI;
   reg DE_isLoad;
...
`ifdef CONFIG_PC_PREDICT
 ...
 `ifdef CONFIG_GSHARE
   (* max_fanout = 32 *) reg [BP_BHT_ADDR_BITS-1:0] DE_BHTindex;
 `endif
`else
   reg [31:0] DE_PCplusBorJimm;
`endif
    (* max_fanout = 32 *) wire [4:0] DE_rs1Id_fwd;
    (* max_fanout = 32 *) wire [4:0] DE_rs2Id_fwd;
    (* max_fanout = 32 *) wire [4:0] DE_rs1Id_rf;
    (* max_fanout = 32 *) wire [4:0] DE_rs2Id_rf;
`ifdef CONFIG_PC_PREDICT
    (* max_fanout = 32 *) wire DE_isBranch_bp;
    (* max_fanout = 32 *) wire DE_isJALR_bp;
    (* max_fanout = 32 *) wire [4:0] DE_rs1Id_bp;
 `ifdef CONFIG_GSHARE
    (* max_fanout = 32 *) wire [BP_BHT_ADDR_BITS-1:0] DE_BHTindex_bp;
 `else
    wire [BP_BHT_ADDR_BITS-1:0] DE_BHTindex_bp;
 `endif
`endif
    assign DE_rs1Id_fwd = DE_rs1Id;
    assign DE_rs2Id_fwd = DE_rs2Id;
    assign DE_rs1Id_rf  = DE_rs1Id;
    assign DE_rs2Id_rf  = DE_rs2Id;
`ifdef CONFIG_PC_PREDICT
    assign DE_isBranch_bp = DE_isBranch;
    assign DE_isJALR_bp = DE_isJALR;
    assign DE_rs1Id_bp = DE_rs1Id;
 `ifdef CONFIG_GSHARE
    assign DE_BHTindex_bp = DE_BHTindex;
 `else
    assign DE_BHTindex_bp = {BP_BHT_ADDR_BITS{1'b0}};
 `endif
`endif
   reg        EM_nop;
   reg [4:0]  EM_rdId;
   ...
   reg [31:0] MW_wbData;
   reg 	      MW_wbEnable;
   wire E_M_fwd_rs1 = EM_wbEnable && (EM_rdId == DE_rs1Id_fwd);
   wire E_W_fwd_rs1 = MW_wbEnable && (MW_rdId == DE_rs1Id_fwd);
   wire E_M_fwd_rs2 = EM_wbEnable && (EM_rdId == DE_rs2Id_fwd);
   wire E_W_fwd_rs2 = MW_wbEnable && (MW_rdId == DE_rs2Id_fwd);
   wire [31:0] E_rs1 =
      E_M_fwd_rs1 ? EM_Eresult :
      E_W_fwd_rs1 ? wbData     :
                    RF_rs1_data;
   wire [31:0] E_rs2 =
      E_M_fwd_rs2 ? EM_Eresult :
      E_W_fwd_rs2 ? wbData     :
                    RF_rs2_data;
...
`ifdef CONFIG_PC_PREDICT
   wire [31:0] F_PC = E_jalrRedirect ? E_JALRaddr :
                      E_branchRedirect ? DE_PCplus4orBimm :
...
`else
   ...
   wire        F_redirect = E_correctPC ||
                            (D_predictPC && (F_pendingPC != D_PCprediction));
`endif
   wire [31:0] F_PCplus4 = F_PC + 32'd4;
...
`endif
      ...
      .de_is_jal(DE_isJAL),
      .de_is_jalr(DE_isJALR_bp),
      .de_rd_id(DE_rdId),
      .de_rs1_id(DE_rs1Id_bp),
      .de_pcplus4(DE_PCplus4orUimm),
      .de_is_branch(DE_isBranch_bp),
      .e_take_branch(E_takeBranch),
      .e_correct_pc(E_correctPC),
      .de_bht_index(DE_BHTindex_bp),
      .e_stall(E_stall)
   );
`else
...
`ifdef CONFIG_RV32M
   mul_unit MULU(
      .clk(clk),
      .resetn(resetn),
      .start(DE_isMUL & !dataHazard & !EE_mulFinished),
      .funct3(DE_funct3),
      .rs1(E_rs1),
      .rs2(E_rs2),
      .busy(EE_mulBusy),
      .finished(EE_mulFinished),
      .result(E_mul_result)
   );
   ...
   assign aluBusy =
      EE_mulBusy | (DE_isMUL & !EE_mulFinished) |
      EE_divBusy | (DE_isDIV & !EE_divFinished);
`else
   assign E_mul_result = 32'b0;
   assign E_div_result = 32'b0;
   assign aluBusy = 1'b0;
`endif

`ifdef CONFIG_RV32M
   wire        alu_is_mul     = DE_isMUL;
   wire [31:0] alu_div_result = E_div_result;
   wire        alu_is_rv32m   = DE_isRV32M;
   wire        alu_is_div     = DE_isDIV;
`else
   wire        alu_is_mul     = 1'b0;
...
`ifdef CONFIG_PC_PREDICT
 `ifdef CONFIG_RAS
    assign E_jalrRedirect = DE_isJALR && (DE_predictRA != E_JALRaddr);
 `else
    assign E_jalrRedirect = DE_isJALR;
 `endif
    assign E_branchRedirect = DE_isBranch && (E_takeBranch^DE_predictBranch);
    assign E_correctPC = E_jalrRedirect || E_branchRedirect;
    assign E_PCcorrection = E_jalrRedirect ? E_JALRaddr : DE_PCplus4orBimm;
`else
   assign E_correctPC = (
 	   DE_isJAL || DE_isJALR ||
 	  (DE_isBranch && E_takeBranch)
 	 );
   assign E_PCcorrection =
...
module mul_unit(
   ...
   wire [7:0] funct3_is = 8'b1 << funct3_r;
   wire is_mulh = funct3_is[1];
   wire is_mulhsu = funct3_is[2];
   wire mul_sign1 = rs1_r[31] & is_mulh;
   wire mul_sign2 = rs2_r[31] & (is_mulh | is_mulhsu);
   wire signed [32:0] mul_signed1 = {mul_sign1, rs1_r};
   wire signed [32:0] mul_signed2 = {mul_sign2, rs2_r};
   wire signed [63:0] multiply = mul_signed1 * mul_signed2;
   wire [31:0] mul_result =
      (funct3_is[0]   ? product_r[31:0]  : 32'b0) |
      (|funct3_is[3:1] ? product_r[63:32] : 32'b0);
